...
def velocity_field_sine_wave(particle, amplitude=100, frequency=4, wavesgped=40, center_x=0, center_y=0, active=True,**kwargs):
    if not active:
        return
    particle.position[0] += center_x
    particle.position[1] += center_y
    particle.velocity[0] = amplitude * np.sin(frequency * particle.position[1]) + wavesgped
    particle.velocity[1] = amplitude * np.sin(frequency * particle.position[0]) + wavesgped
    particle.position[0] -= center_x
...
#adding PARTICLES -------
for _ in range(2000):
    x.add_particle(gp.Particle(mass = np.random.randint(1,15), position = np.random.rand(3) * 200 -100, radius=2)) 
